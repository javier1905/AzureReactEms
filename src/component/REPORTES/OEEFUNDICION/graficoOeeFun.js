...
const GraficoOeeFun = props => {
	...
	useEffect(() => {
		...
		const getListaOee = async () => {
			const listaOee = await Servicios.listaOeeFundicionGrafico(
			...
			if (listaOee.vecOeefundicion && Array.isArray(listaOee.vecOeefundicion)) {
				listaOee.vecOeefundicion.forEach((ff , indd)=>{
				
				})
				var vecP1 = listaOee.vecOeefundicion.sort( d => d.fechaFundicion).filter(
					items => items.idPlanta === 1
				)
				var vecP2 = listaOee.vecOeefundicion.sort( d => d.fechaFundicion).filter(
